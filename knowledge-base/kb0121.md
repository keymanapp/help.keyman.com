...
The challenge with using an older version of the keyboard is that Keyman will be prompted to update to the new version every now and then. Keyman for Android and iOS will automatically check and provide updates for any installed keyboards; Keyman for macOS and Linux currently updates keyboards automatically. Keyman for Windows allows disabling automatic updates, please refer to:
...
5) Check [see-also](#toc-see-also) section to install the KMP file onto the device.
...
6) Change the `version=1.2` to the desired version
...
8) Check [see-also](#toc-see-also) section to install the KMP file onto the device.
