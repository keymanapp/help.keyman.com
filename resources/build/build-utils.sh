...
# Based on Keyman repo (removed VERSION code)
#
...
# Uses tier.txt instead of TIER.md
function findTier() {
    local TIER_TXT="$KEYMAN_ROOT/tier.txt"
    TIER=`cat $TIER_TXT | tr -d "[:space:]"`
    [[ "$TIER" =~ ^(TIER_DEVELOPMENT|TIER_STAGING|TIER_PRODUCTION|TIER_TEST)$ ]] || {
        echo "Invalid tier.txt file: expected TIER_DEVELOPMENT, TIER_STAGING, TIER_PRODUCTION or TIER_TEST."
        exit 1;
    }
}
