<?php
  ...
  head([
    'title' => "Function: indexOutput"
  ]);

  require_once('includes/__docheader.php');
?>

<h1 class="title" id="KIO">indexOutput (KIO)</h1>

<h2 id="Summary" name="Summary">Summary</h2>

<p>Index-based output:  Outputs a mapped character according to a previous selection from a <code>keyman.interface.any()</code>
...
<?php
syntax("keyman.interface.indexOutput(nd, store, index, Pelem);", "KeymanWeb.KIO(nd, store, index, Pelem);");
params("nd", "number", "The number of characters to be overwritten.",
  "store", "string", "The store string to be used for mapping a character previously detected via <code>keyman.interface.any()</code>".
  ...
  "index", "number", "The index id to which the detected character's index in the original matching array ".
    "was saved.  The index id should be incremented by 1 relative to the value specified in <code>keyman.interface.any()</code>.",
...
<p>facilitates mapping the characters <code>'abcde'</code> to their respective entry in the output store <code>'αβγδε'</code>.
  <code>keyman.interface.indexOutput()</code>'s role in this process is usually optimized out into individual mappings for performance reasons,
...
<?=codebox("keyman.interface.indexOutput(0, 'αβγδε', 1, Pelem)")?>
<p>would suffice, overwriting no characters and using the previously-matched index (id = 0 + 1).  Note that the
  <code>keyman.interface.indexOutput()</code></a> call must increment the id by 1 due to Keyman's keyboard language rules, which corresponds
  directly to the <code class="language-keyman">index</code> call in the keyboard source above.</p>

<h2>See also</h2>

<p><a href="any"><code>keyman.interface.any()</code></a></p>