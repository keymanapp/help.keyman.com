...
implemented a few of the vowel symbols in this keyboard: <img src="<?= cdn('img/developer/100/ipa.gif')?>"></p>
...
<pre><code class="language-keyman">store( choices ) '1234567890'
...
<pre><code class="language-keyman">group( first )
...
<pre><code class="language-keyman">c IPAMenu.kmn
c
...
<p>When a key is pressed, execution begins at the group indicated by the <code class="language-keyman">begin</code> statement, in this case the <code class="language-keyman"><var>first</var></code>
group. Because this group does not specify <code class="language-keyman">using keys</code>, it is limited to context manipulation only: the output of this rule is 
...
<p>Let's suppose the <kbd>a</kbd> key has been pressed with no context. The <code class="language-keyman"><var>first</var></code> group will have nothing to match on, so the
<code class="language-keyman">nomatch</code> rule fires and passes control to the <code class="language-keyman"><var>main</var></code> group. Here the "a" key is matched, and the
<code class="language-keyman"><var>a_menu</var></code> store is output, displaying the menu of a-like characters.</p>

<p>Now the user is presented with a menu of options to choose from. Suppose he types <kbd>1</kbd> Once again the <code class="language-keyman"><var>first</var></code>
group gains control first, but this time matches the first rule, with the <code class="language-keyman"><var>a_menu</var></code> string on the context, so control is passed 
to the <code class="language-keyman"><var>a_menu</var></code> group to handle the keystroke. Here the <kbd>1</kbd> is matched as an entry in the 
<code class="language-keyman"><var>choices</var></code> store, and the corresponding character in the <code class="language-keyman"><var>a_chars</var></code> store - in this case "æ" - is output. 
Finally, control goes from here to the <code class="language-keyman"><var>final</var></code> group, which fails to match anything in the context (which now includes 
...
Backspace to delete it - this is simply done with a rule matching <code class="language-keyman">[K_BKSP]</code> and outputting <code class="language-keyman">nul</code>.</p>
...
can press a number key indicating a menu entry that is not there - this situation will be matched by the <code class="language-keyman">any(choices)</code> rule. 
The other occasion is if the user presses any other key which is not valid for selecting a menu option. This is handled by the <code class="language-keyman">nomatch</code> 
rule in the group. For both these cases we want the output to be the same: a <code class="language-keyman">beep</code>, and remain at the menu. To do this we will use 
deadkeys as error flags, one for each menu. By padding the <code class="language-keyman"><var>a_char</var></code>, <code class="language-keyman"><var>e_char</var></code> and 
<code class="language-keyman"></var>o_char</var></code> stores to 10 characters with these deadkeys, the output for this first situation will be the error flag. 
Similarly, we can output these deadkeys in the <code class="language-keyman">nomatch</code> rules, to mark an error.</p>

<p>The actual error handling is now done with the <code class="language-keyman"><var>final</var></code> group, which matches the error flags on the output 
and outputs the <code class="language-keyman">beep</code> and the appropriate menu again.</p>
