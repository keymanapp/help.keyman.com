...
"/developer/language/reference/any" title="any statement"><code>any()</code></a> and <a href="/developer/language/reference/index" title=
"index statement"><code>index()</code></a> statements. We create a store with a <a href="/developer/language/reference/store"
title="store statement"><code>store()</code> statement</a>:</p>

<pre><code>store(vowels) "aeiou"
</code></pre>

<p>This creates a store called <code><var>vowels</var></code>, which contains the five lowercase vowels. Note that we could also have written the
content of the store using ANSI or Unicode character codes, in the same way as the output.</p>

<p>The <code>any()</code> statement is used to match a character from a specific store. For example, the following rule will
replace any vowel with a period, when used with the store above:</p>

<pre><code>+ any(vowels) > "."
</code></pre>

<p>The <code>any()</code> statement can be used in the context or in the key part of a rule. It cannot be used in the
output.</p>

<p>The second statement that is used with stores is the <code>index()</code> statement. It is usually used in the output of
...
<code>any()</code> statement. This is best shown with an example; this rule will convert all input to uppercase:</p>
    
<pre><code>store(lowercase) "abcdefghijklmnopqrstuvwxyz"
store(uppercase) "ABCDEFGHIJKLMNOPQRSTUVWXYZ"

+ any(lowercase) > index(uppercase,1)
</code></pre>

<p>When a letter, such as <kbd>j</kbd> is typed, the <code>any()</code> statement finds its position in the
<code><var>lowercase</var></code> store; the <code>index()</code> statement then gets this index from the <code>any()</code>
statement, and outputs the character at the same position in the <code><var>uppercase</var></code> store, in this case <samp>J</samp>.</p>

<p>The <code>index()</code> statement has two parts: the store from which it takes the output character, and the offset of
the <code>any()</code> statement that it gets the character position from. This offset is found by counting the characters and 
statements in the context and key parts of the rule up to the <code>any()</code> statement. Again, a few examples may help to
illustrate this:</p>

<pre><code>"a" + any(somestore) > index(otherstore,2)         c The 'any' statement
                                                   c is character #2
...
<code>group()</code> statement is repeated below:</p>
...
<pre><code>"'" + "y" > U+00FD    c Acute-accented Y
"'" + "Y" > U+00DD
