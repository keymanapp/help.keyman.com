...
9) Add dynamically rendered keyboard layouts ([more details](#toc-dynamically-constructed-keyboard-images))
...
* If you use the recommended dynamically rendered on screen keyboard with the `<div id='osk'>` and related elements (above), you should remove any reference to images of the keyboard from the PHP version of the documentation (for example, those generated by the On Screen Keyboard export tool in Keyman Developer). These files are then also not required in the help folder.
